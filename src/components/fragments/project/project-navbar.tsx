...
const getProjectPageTitle = (pathname: string): string => {
  ...
  if (pathname.endsWith('/topics')) {
    return 'Topic Modeling';
  }

  if (pathname.endsWith('/sentiment')) {
    return 'Sentiment Trend';
  }

  if (pathname.endsWith('/emotion')) {
    return 'Emotion Analysis';
  }

  if (pathname.endsWith('/influencers')) {
    return 'Influencers';
  }

  if (pathname.endsWith('/communities')) {
    return 'Communities';
  }
...
const ProjectNavbar = ({ workspace, project }: ProjectNavbarProps) => {
    ...
    <header className="relative z-10 flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-6">
      <div className="flex min-w-0 items-center gap-2 text-sm">
        <Link
          ...
          className="max-w-40 truncate font-medium text-slate-500 transition hover:text-slate-900"
        >
          {workspace.name}
        </Link>
        ...
        <Link
          to={`/workspaces/${workspace.id}/projects/${project.id}/overview`}
          className="max-w-48 truncate font-medium text-slate-500 transition hover:text-slate-900"
        >
          {project.name}
        </Link>
